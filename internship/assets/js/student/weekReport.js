...
jQuery(document).ready(function(){
	...
	jQuery(grid_selector).jqGrid({
	    ...
	    datatype: "json",
		height: 372,
		colNames:['月报ID','发送日期','姓名','学习内容','学习收获', '问题反馈','状态','操作'],
		colModel:[
		...
		beforeRequest:function(){
			...
			var newUrl = apiUrl+"mobile/dailyPaper?page="+currentpage+"&limit="+rowNum+"&type=1&token="+cookie.get("token");
            if(createTime !== ""){
		...
		loadComplete : function() {
            ...
            for ( var i = 0; i < ids.length; i++) {
                ...
                if(columnData.state == 0){
                	...
                	be = '打回';
                	ce = '<button id="update_report_' + ids[i] + '" class="btn btn-info btn-xs" onclick="update_dayReport_fn(this);" style="float:left;"><i class="icon-pencil align-top bigger-125"></i>修改周报</button>';
                }
...
function new_dayReport(){
	...
	var endDate;
	if(weekDay>0){
		startDate = new Date(now.getTime()-24*60*60*1000*(weekDay-1)).Format("yyyy-MM-dd");
		endDate = new Date(now.getTime()+24*60*60*1000*(7-weekDay)).Format("yyyy-MM-dd");
	}else{
		startDate = new Date(now.getTime()-24*60*60*1000*6).Format("yyyy-MM-dd");
