...
jQuery(document).ready(function(){
	...
	jQuery(grid_selector).jqGrid({
		...
		beforeRequest:function(){
			...
			var newUrl = apiUrl+"mobile/dailyPaper?page="+currentpage+"&limit="+rowNum+"&type=2&token="+cookie.get("token");
            if(createTime !== ""){
		...
		loadComplete : function() {
            ...
            for ( var i = 0; i < ids.length; i++) {
                ...
                if(columnData.state == 0){
                	...
                	be = '打回';
                	ce = '<button id="update_report_' + ids[i] + '" class="btn btn-info btn-xs" onclick="update_dayReport_fn(this);" style="float:left;"><i class="icon-pencil align-top bigger-125"></i>修改月报</button>';
                }
...
function new_dayReport(){
	...
	var startDate = new Date(new Date().setDate(1)).Format("yyyy-MM-dd");
	var endDate = new Date(new Date(new Date(start).setMonth(start.getMonth()+1)).setDate(0)).Format("yyyy-MM-dd");
