...
jQuery(document).ready(function(){
    ...
    jQuery(grid_selector).jqGrid({
        ...
        datatype: "json",
        height: 372,
        colNames:['申请时间','开始时间','结束时间','请假事由','状态','操作'],
        colModel:[
            // {name:'id',index:'id', width:150,editable:false,hidden: true},
            {name:'createTime',index:'createTime', width:150,editable:false},
            {name:'leaveStartDate',index:'leaveStartDate', width:100,editable:false},
            {name:'leaveEndDate',index:'leaveEndDate',width:100, editable:false},
        ...
        beforeRequest:function(){
            ...
            var newUrl = apiUrl+"mobile/leaveApply?page="+currentpage+"&limit="+rowNum+"&token="+cookie.get("token");
            if(createTime !== ""){
    ...
    $('#makeSureSend').on('click',function () {
        ...
        var leaveStartDate = $('#form-field-3').val();
        var leaveEndDate = $('#form-field-4').val();
        var remark = $('#form-field-5').val();

        $.ajax({
            url: apiUrl + 'mobile/leaveApply',
            type: 'POST',
            data: {
...
function update_student_dialog_fn(e) {
    ...
    var startTime = $(e).parent().siblings().eq(2).text();
    var endTime = $(e).parent().siblings().eq(3).text();
    var remark = $(e).parent().siblings().eq(4).text();
...
function update_student() {
    ...
    var leaveStartDate =  $("#editStartTime").val();
    var leaveEndDate =  $("#editEndTime").val();
    ...
    $.ajax({
        ...
        url: apiUrl + "mobile/leaveApply?page=1&limit=10",//路径
        data: {
            "id": editApprovalId,
            "leaveStartDate":leaveStartDate,
            "leaveEndDate":leaveEndDate,
...
function delete_group_fn(e) {
    ...
    $.ajax({
        ...
        url: apiUrl + "mobile/leaveApply?id=" + choose_student_id,//路径
        success: function (data) {//返回数据根据结果进行相应的处理
            if (data.code === 0) {
                alert("删除成功!");
                jQuery("#grid-table").jqGrid().trigger("reloadGrid");
